...
// Prototype static server — mirrors repo scripts/serve.mjs pattern, rooted at this prototype.
const root = resolve(import.meta.dirname, "..");
const portFlag = process.argv.indexOf("--port");
const requestedPort = portFlag >= 0 ? Number(process.argv[portFlag + 1]) : Number(process.env.PORT ?? 4173);
// --port 0 binds an ephemeral port (harness convenience); anything else falls back to 4173.
...
server.listen(port, "127.0.0.1", () => {
  ...
  console.log("oci-reference preview: http://127.0.0.1:" + bound);
});
