...
test('the boot cover keeps the house rules while it briefs the race', async () => {
  ...
  /* The renderer status line left the footer at the owner's direction, whole:
     the sentence, its crossfade and its hairline said nothing the plates over
     them were not already saying, and the fleet plate takes the room. Nothing
     on the layer may claim the race is loading: it is already built by the
     time this layer paints. */
  assert.doesNotMatch(brief, /already loaded/);
  assert.doesNotMatch(cover, /\.status(?:Stack|Bar|Fill)?\s*[,{]/);
  assert.doesNotMatch(cover, /@keyframes statusWarm/);

  /* Display text carries no periods, here as everywhere else on the page. */
  for (const line of ['Start the race']) {
