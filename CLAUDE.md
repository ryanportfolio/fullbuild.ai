...
- Never claim visual/UI verification you didn't actually perform. Real visual checks available: `tools/visual/` (Playwright cross-browser + visual-diff + motion capture) and the Figma plugin (`get_design_context` / `get_variable_defs` / `get_screenshot`) — see `.claude/reference/commands.md`.
