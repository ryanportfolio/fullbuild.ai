...
// Template-only checks: the bootstrap/ generator ships with the claude-starter
// template and is deleted when a project is spawned from it. Spawned repos have
// no bootstrap/ directory, so these checks only apply when it is present.
if (exists("bootstrap")) {
  ...
  for (const wrapper of ["bootstrap/new-claude-project.ps1", "bootstrap/new-claude-project-ui.ps1"]) {
    if (!exists(wrapper)) {
      failures.push(`${wrapper}: generator wrapper is missing`);
      continue;
    }
