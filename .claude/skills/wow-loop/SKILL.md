---
description: Multi-agent perfection loop for any creative or technical deliverable (site, page, animation, deck, PDF, design system). Orchestrates recon, a single authoritative spec, a sole implementer, adversarial screenshot-verified critique, and fix-reverify rounds until a strict evidence gate passes. Use when the user says /wow-loop, asks to make something "utterly perfect", "dial it to 11", wants "wow factor", says "loop until it's flawless", or asks for the perfection methodology on a new subject.
---
...
`wow-loop: subject-agnostic multi-agent perfection loop (recon, spec, sole implementer, adversarial screenshot-verified critique, fix-reverify until strict gate passes)` names the target when invoked as a slash command; otherwise take the deliverable from conversation context.
