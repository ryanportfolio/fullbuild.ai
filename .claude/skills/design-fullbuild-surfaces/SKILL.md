...
1. The code on the surface you are touching. `src/app/globals.css` opens with the binding constraint contract for The Working Set; each prototype, whether under `public/prototype/` or `src/app/prototype/`, owns its own tokens and its own ban list.
