 ...
 *
 * The two shapes here split on cost. `briefFacts` walks the fixes once and is
 * built at mount; `windReadingAt` is called every frame and writes into a
 * caller-owned object, the same contract `poseAt` and `windAt` keep.
 */
import { windAt } from "./interpolate";
import { polarFrac } from "./sim";
import type { Course, Fix, RaceData, WindSample } from "./types";
...
export function windReading(): WindReading {
  return { t: 0, twd: 0, tws: 0, biasSeconds: 0, favored: "square" };
}
 ...
 *
 * Bias in seconds is the line's length across the wind divided by the speed
 * the fleet makes at its beat angle: `lineLength * sin(|twd|) / beatSpeed`,
 * with `beatSpeed = polarFrac(beatTwa) * tws` off the same polar the sim sails
 * (POLAR_TWA / POLAR_FRAC in lib/layline/sim.ts). At the shipped race's 46.0
 * degree beat and 7.2 m/s mean that is 0.846 of the breeze, 6.09 m/s.
 *
...
export function windReadingAt(
  ...
  out.tws = tws;
  out.biasSeconds =
    beatSpeed > 0 ? (facts.lineLength * Math.sin(Math.abs(twd) * DEG)) / beatSpeed : 0;
  out.favored = twd > SQUARE_DEG ? "boat" : twd < -SQUARE_DEG ? "pin" : "square";
  return out;
}

/**
 * The prestart breeze as a polyline, for the trace under the dial. Sampled off
 * `windAt` rather than off `race.wind` directly so the curve drawn is the curve
 * the replay reads between the 1 Hz samples.
 */
export function prestartTrace(
  race: RaceData,
  steps: number,
): { t: number; tws: number }[] {
...
/** Course x to the diagram's own -1..1 across the line, clamped to the ends. */
export function acrossLine(x: number, half: number): number {
  const u = half > 0 ? x / half : 0;
  return u < -1 ? -1 : u > 1 ? 1 : u;
}

export function lineEndsOf(course: Course): { pinX: number; boatX: number } {
